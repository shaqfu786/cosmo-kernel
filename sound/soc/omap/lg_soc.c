...
static int sdp4430_mcpdm_startup(struct snd_pcm_substream *substream)
{
	...
	if (twl6040_power_mode) {
		 ...
		 */
		ret = cdc_tcxo_set_req_int(CDC_TCXO_CLK3, 1);
		if (ret) {
...
#if defined (CONFIG_MACH_LGE_P2_LU5400)			
	else if( be_id == OMAP_ABE_DAI_MODEM ) {
...
#ifdef CONFIG_SND_OMAP_SOC_DMIC
...
MODULE_DESCRIPTION("ALSA SoC LG");
MODULE_LICENSE("GPL");
