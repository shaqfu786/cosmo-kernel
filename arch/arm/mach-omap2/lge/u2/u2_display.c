...
static struct lge_dsi_panel_data dsi2_hitachi_u2_panel = {
...
static struct omap_dss_device hitachi_u2_device =
{
	.name	= "2lcd",
	.driver_name	= "tx11d108vm_panel",
	.type	= OMAP_DISPLAY_TYPE_DSI,

	.data = &dsi2_hitachi_u2_panel,
	.phy.dsi		= {
		.clk_lane	= 1,
		.clk_pol	= 0,
		.data1_lane	= 3,
	...
	.clocks = {
		...
		.dsi = {
			.regn		= 17,  /* Fint = 1.92 MHz */
			.regm		= 211,	/* CLKIN4DDR = 768.0 MHz */
			.regm_dispc	= 4,  /* PLL2_CLK1 = 153.6 MHz */
			.regm_dsi	= 6,  /* PLL2_CLK2 = 109.7 MHz */
...
static struct lge_dsi_panel_data dsi2_hitachi_u2_p760_panel = {
	.name			= "hx8389_panel",
	.reset_gpio		= GPIO_LCD_RESET,
	.use_ext_te 		= false,
	.ext_te_gpio		= GPIO_LCD_EXT_TE,
...
static struct omap_dss_device hitachi_u2_p760_device =
{
	.name	= "2lcd",
	.driver_name	= "hx8389_panel",
	.type	= OMAP_DISPLAY_TYPE_DSI,

	.data = &dsi2_hitachi_u2_p760_panel,
	.phy.dsi		= {
		.clk_lane	= 1,
		.clk_pol	= 0,
		.data1_lane	= 3,
...
static struct omap_dss_device hdmi_device = {
	...
	.clocks	= {
		...
		.hdmi	= {
			.regn	= 15,
			.regm2	= 1,
// HDMI PLL supports maximum pixel clock of 148.5MHz
#if 1
...
int __init u2_dss_init(void)
{
	...
	if(system_rev == 0){
		...
		lcd2_info = hitachi_u2_p760_device;
	}
#endif
...
lge_machine_initcall(u2_dss_init);

/*
 * Set the FB size to 10MB for upto three buffers for U2
 * Set the FB size to 8MB for upto two buffers for U2
...
int __init u2_fb_init(void)
{
	return lge_set_fb_info(&fb_pdata);
}

lge_machine_initcall(u2_fb_init);

static void u2_dsi_phy_init(void)
{
	u32 reg;
