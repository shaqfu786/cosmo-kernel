...
TOUCH_INFO_MSG("fw_rev [%d] vs min ver to allow fw-upgrade [%d]\n",ts->fw_info.fw_rev, ALLOW_MIN_VER);
	if(ts->fw_info.fw_rev >= ALLOW_MIN_VER){
		...
		TOUCH_INFO_MSG("FW ver in Touch Ic is too low ver. So, FW-upgrade is not executed\n");
		goto out;
	}

#elif defined(CONFIG_TOUCHSCREEN_COMMON_SYNAPTICS_S3200)
	// S3200 series [START] ----------------------------------------
...
#ifdef CONFIG_TOUCHSCREEN_COMMON_SYNAPTICS_S3200        /*                                                                                        */
	u8 fw_version;
	fw_version = simple_strtol(&ts->fw_info.config_id[1],NULL,10);
	ret = sprintf(buf, "%ld\n", fw_version);
#else
	ret = sprintf(buf, "%d\n", ts->fw_info.fw_rev);
#endif
