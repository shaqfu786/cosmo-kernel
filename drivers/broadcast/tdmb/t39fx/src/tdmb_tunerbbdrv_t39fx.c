/* drivers/broadcast/t39fx/src/tdmb_tunerbbdrv_t39fx.c
...
#include "../inc/tdmb_tunerbbdrv_t39fxdef.h"
...
**    5.   Local Typedef
-------------------------------------------------------------*/
typedef enum t39fx_service_type
{
	T39FX_DAB = 1,
	T39FX_DMB = 2,
	T39FX_VISUAL =3,
...
**    6.   Global Variables
-------------------------------------------------------------*/
t39fx_service_type	serviceType;

/* ----------------------------------------------------------
**    7.   Static Variables
-------------------------------------------------------------*/
static ST_SUBCH_INFO g_stSubInfo;
//static t39fx_service_type serviceType;

void tunerbb_drv_t39fx_set_userstop(int mode)
{
	tdmb_t39fx_set_userstop( mode );
//	INTERFACE_USER_STOP(TDMB_RFBB_DEV_ADDR);
}

int8 tunerbb_drv_t39fx_power_on(void)
{
	int rc;
	rc = tdmb_t39fx_power_on( );
	return rc;
}

int8 tunerbb_drv_t39fx_power_off(void)
{
	int rc;
	rc = tdmb_t39fx_power_off( );
	return rc;
}

int8 tunerbb_drv_t39fx_select_antenna(unsigned int sel)
{
	int rc;
	rc = tdmb_t39fx_select_antenna(sel);
	return rc;
}

int8 tunerbb_drv_t39fx_reset_ch(void)
{
...
int8 tunerbb_drv_t39fx_stop(void)
{
	INC_UINT8 nRet;
	//printk("[T39fx] INC_STOP\n");
	nRet = INC_STOP(TDMB_RFBB_DEV_ADDR);
	if(nRet != INC_SUCCESS)
	{
		printk("[T39fx] INC_STOP Error\n");
	}
	return nRet;
}
...
int8 tunerbb_drv_t39fx_get_msc_ber(uint32 *msc_ber)
{
...
int8 tunerbb_drv_t39fx_set_channel(int32 freq_num, uint8 subch_id, uint8 op_mode)
{
	return tunerbb_drv_t39fx_multi_set_channel(freq_num, 1, &subch_id, &op_mode);
}

int8 tunerbb_drv_t39fx_re_syncdetector(uint8 op_mode)
{
	int8 ret_val = INC_ERROR;
	
	if(op_mode != T39FX_ENSQUERY)
	{		
...
int8 tunerbb_drv_t39fx_re_sync(void)
{
	return INTERFACE_RE_SYNC(TDMB_RFBB_DEV_ADDR);
}

int8 tunerbb_drv_t39fx_control_fic(uint8 enable)
{
	return INC_ERROR;
}


uint32 tunerbb_drv_t39fx_get_freq(int nFreqIndex)
{
...
int8 tunerbb_drv_t39fx_tune(int nFreqNo)
{
	uint32 ulFreq;
	INC_UINT8 nRet = INC_ERROR;

	ulFreq = tunerbb_drv_t39fx_get_freq(nFreqNo);
	//printk("tunerbb_drv_t39fx_tune nFreqNo = %d, ulFreq = %d \n", nFreqNo, ulFreq);
...
int8 tunerbb_drv_t39fx_get_datatype(uint8 subchid, uint8* datatype)
{
	int i;
...
int8 tunerbb_drv_t39fx_get_datathreshold(uint8 subchid, uint32* threshold)
{
	int i;
...
int8 tunerbb_drv_t39fx_process_multi_data(uint8 subch_cnt,uint8* input_buf, uint32 input_size, uint32* read_size)
{
	ST_FIFO*	pMultiFF;
	uint32	nDataSize;
	int			i;
	...
	if(INC_MULTI_FIFO_PROCESS(input_buf, input_size))
	{
		for(i=1;i<=subch_cnt+1;i++)
		{
			...
			************************************************/
			...
			{
				nDataSize = INC_QFIFO_GET_SIZE(pMultiFF);
				if(tunerbb_drv_t39fx_get_datathreshold(pMultiFF->unSubChID, &threshold)==INC_ERROR)
				{
					return INC_ERROR;
				}
			}
...
int8 tunerbb_drv_t39fx_get_multi_data(uint8 subch_cnt, uint8* buf_ptr, uint32 buf_size)
{
	ST_FIFO*	pMultiFF;
	uint32	nDataSize;
	int			i;
	...
	for(i=1;i<=subch_cnt+1;i++)
	{
		...
		************************************************/
		...
		{
			nDataSize = INC_QFIFO_GET_SIZE(pMultiFF);
			if(tunerbb_drv_t39fx_get_datatype(pMultiFF->unSubChID, &datatype)==INC_ERROR)
			{
				return INC_ERROR;
			}
			if(tunerbb_drv_t39fx_get_datathreshold(pMultiFF->unSubChID, &threshold)==INC_ERROR)
	...
	{
		printk("[T39fx] tunerbb_drv_t39fx_get_multi_data() Error [read_size:%d, buf_size:%d]",read_size,buf_size);
		return INC_ERROR;		
	}
	else
	{
...
void tunerbb_drv_t39fx_start_tii(void)
{
	INC_TII_START(TDMB_RFBB_DEV_ADDR);
	//INC_MSG_PRINT(1,"=== INC_TII_START(%d) \n");
}

void tunerbb_drv_t39fx_stop_tii(void)
{
