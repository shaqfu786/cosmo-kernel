 ...
 *
 * $Id: dhd_sdio.c 321750 2012-03-16 17:37:17Z $
 */
...
dhdsdio_rxglom(dhd_bus_t *bus, uint8 rxseq)
{
	...
	if (bus->glom) {
		...
		if ((uint8)(txmax - bus->tx_seq) > 0x40) {
			DHD_ERROR(("P:1:%s: got unlikely txmax %d with tx_seq %d:tx_max[%d]\n",
			           __FUNCTION__, txmax, bus->tx_seq, bus->tx_max));
	...
	*finished = FALSE;
	...
	for (rxseq = bus->rx_seq, rxleft = maxframes;
		...
		if (dhd_readahead && bus->nextlen) {
			...
			/* Check window for sanity */
			if ((uint8)(txmax - bus->tx_seq) > 0x40) {
					DHD_ERROR(("P:2:%s: got unlikely txmax %d with tx_seq %d:tx_max[%d]\n",
						__FUNCTION__, txmax, bus->tx_seq, bus->tx_max));
#ifdef CONFIG_COMMON_PATCH
					txmax = bus->tx_seq + 2;
#else
		...
		/* Check window for sanity */
		if ((uint8)(txmax - bus->tx_seq) > 0x40) {
			DHD_ERROR(("P:3:%s: got unlikely txmax %d with tx_seq %d:tx_max[%d]\n",
			           __FUNCTION__, txmax, bus->tx_seq, bus->tx_max));
#ifdef CONFIG_COMMON_PATCH
			txmax = bus->tx_seq + 2;
#else
